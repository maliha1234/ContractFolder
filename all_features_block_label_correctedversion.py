...
def instructions(dict,label,smart_contract_name):
    #Max number of instruction of a single block
    len(dict[key])
...
def get_opc_dict(dir,dict,label,smart_contract_name, first_six_features):
    ...
    file = 'all_features_with_block_label_sep6.csv'
    for key in dict:
        try :
            write = smart_contract_name+","+ str(key) + ","+str(label[key])+ "," + first_six_features +"," + str(dict1[key]) +","+str(dict2[key])+ ","+ str(dict3[key])+ ","+str(dict4[key])+ ","+str(dict5[key])+ ","+str(dict6[key])+ ","+str(dict7[key])+ ","+str(dict8[key]) + ","+str(dict9[key])+ ","+str(dict10[key])+ ","+str(dict11[key])+"\n"
...
def getlabel(dir):
    ...
    with open(project_dir+"/"+"label_with_block_smart_contract_debo.csv") as csv_file:
